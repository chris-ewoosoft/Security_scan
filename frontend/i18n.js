window.SecurityPortalI18n = (() => {
  ...
  const dict = {
    vi: {
      ...
      "home.staleApiHint": "API đang chạy bản cũ (stamp {actual}, cần {expected}). Chạy: git checkout cursor/fix-cvmanager-auth-zero-redirect-d50f && docker compose build --no-cache api && docker compose up -d --force-recreate api nginx. Mở http://localhost/api/v1/scans/build-info để kiểm tra stamp.",
      "home.zeroRedirectHint": "Lỗi MaxAutomaticRedirections=0 — binary API chưa được rebuild đúng. Xóa URL ?id=…, chạy lại scan mới sau khi build-info trả stamp {expected}.",
    ...
    en: {
      ...
      "home.staleApiHint": "API is running an old build (stamp {actual}, need {expected}). Run: git checkout cursor/fix-cvmanager-auth-zero-redirect-d50f && docker compose build --no-cache api && docker compose up -d --force-recreate api nginx. Open http://localhost/api/v1/scans/build-info to verify stamp.",
      "home.zeroRedirectHint": "MaxAutomaticRedirections=0 error — API binary was not rebuilt correctly. Clear ?id=… from the URL, start a new scan after build-info returns stamp {expected}.",
      "home.apiStamp": "API build",
